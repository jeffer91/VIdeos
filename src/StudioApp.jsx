...
function slideValidation(slide = {}) {
  return slide.validation || {
    title: !!slide.title?.trim(),
    body: !!slide.body?.trim(),
    content: !!slide.content?.trim(),
    reading: !!slide.reading?.trim(),
    bodyBulleted: !slide.body?.trim() || isBulleted(slide.body),
    contentBulleted: !slide.content?.trim() || isBulleted(slide.content),
  };
}
...
function normalizeProject(project) {
  ...
  return {
    ...
    slides: (project.slides || []).map((slide) => ({
      ...
      validation: slideValidation(slide),
    })),
  };
}
...
export default function StudioApp() {
  ...
  const currentHasReading = !!currentSlide?.reading?.trim();
  ...
  const missingReadingCount = useMemo(
    () => (project?.slides || []).filter((slide) => !slide.reading?.trim()).length,
    [project],
  );
  ...
  useEffect(() => {
    ...
    const initialise = async () => {
      try {
        const activeRaw = await getActiveProject();
        const active = normalizeProject(activeRaw);
  ...
  async function copyAiRules() {
    try {
      ...
      setNotice('Reglas para IA copiadas al portapapeles.');
    } catch {
      const field = document.createElement('textarea');
      field.value = AI_FORMAT_RULES;
  ...
  function downloadAiRules() {
    downloadText(AI_FORMAT_RULES, 'reglas-videos-studio-ia.txt');
    setNotice('Reglas descargadas en TXT.');
  }
  ...
  async function saveContentProject() {
    ...
    if (!updating) {
      ...
      setCurrentSlideIndex((index) => Math.min(index, Math.max(0, nextProject.slides.length - 1)));
    }
    ...
    setNotice(
      updating
        ? `Proyecto actualizado. ${result.slides.filter((slide) => slide.reading?.trim()).length}/${result.slides.length} diapositivas tienen LECTURA.`
        : `Proyecto creado con ${result.slides.length} diapositivas.`,
    );
    setError('');
  }
  ...
  async function startRecording() {
    ...
    if (!currentHasReading) {
      setError(`La diapositiva ${currentSlide.number} no tiene LECTURA. Agrégala en Contenido antes de grabar.`);
      return;
    }
  ...
  async function acceptCurrentTake() {
    ...
    if (currentSlideIndex < project.slides.length - 1) {
      ...
      const pending = project.slides.findIndex(
        (slide) => slide.number !== currentSlide.number && !takes[slide.number]?.accepted,
      );
    ...
    <div className="studio-app">
      <header className="studio-header">
        ...
        <div className="studio-progress">
          {project ? (
            <><strong>{acceptedCount}/{project.slides.length}</strong><span>aceptadas</span></>
          ) : (
            <><strong>Nuevo</strong><span>proyecto</span></>
          )}
        </div>
      </header>
      ...
      <main className="studio-main">
        {view === 'content' && (
          <section className="content-screen enhanced-content-screen">
            <div className="screen-heading compact-content-heading">
              <div>
                <span className="eyebrow">PANTALLA 1</span>
                <h1>Cargar contenido</h1>
                <p>TÍTULO + CUERPO + CONTENIDO se muestran. LECTURA es exclusivamente para el prompter.</p>
              </div>
              {project && <button className="ghost-button" onClick={() => navigate('recording')}>Volver a grabación</button>}
            </div>

            <div className="ai-rules-card">
              <div className="ai-rules-copy">
                <span className="eyebrow">FORMATO PARA IA</span>
                <strong>Reglas de contenido y lectura</strong>
                <small>CUERPO y CONTENIDO siempre por puntos · LECTURA = lo que tú lees.</small>
              </div>
              <div className="format-flow">
                <span><b>SE VE</b> TÍTULO + CUERPO + CONTENIDO</span>
                <span><b>SE LEE</b> LECTURA</span>
              </div>
              <div className="ai-rules-actions">
                <button className="secondary-button" onClick={copyAiRules}>Copiar reglas</button>
                <button className="secondary-button" onClick={downloadAiRules}>Descargar TXT</button>
              </div>
            </div>

            <div className="content-grid enhanced-content-grid">
              <div className="paste-card">
                <div className="card-title-row">
                  <strong>Contenido fuente</strong>
                  <label className="file-button">Cargar TXT<input type="file" accept=".txt,text/plain" onChange={loadTextFile} /></label>
                </div>
                <textarea
                  value={rawText}
                  onChange={(event) => setRawText(event.target.value)}
                  placeholder={'DIAPOSITIVA 1\nTÍTULO:\n...\n\nCUERPO:\n- punto visual\n\nCONTENIDO:\n- dato visual\n\nLECTURA:\nTexto que leerás en el prompter.\n\n//'}
                />
              ...
              <div className="parse-card">
                <div className="card-title-row">
                  ...
                  {parseResult && <span className="count-badge">{parseResult.slides.length} detectadas</span>}
                </div>
                {!parseResult && <div className="empty-state">Procesa el texto para comprobar TÍTULO, CUERPO, CONTENIDO y LECTURA.</div>}
                {parseResult && (
                  <>
                    {!!parseResult.errors.length && (
                      <div className="validation error-box">{parseResult.errors.map((item) => <div key={item}>• {item}</div>)}</div>
                    )}
                    {!!parseResult.warnings.length && (
                      <div className="validation warning-box">{parseResult.warnings.slice(0, 6).map((item) => <div key={item}>• {item}</div>)}</div>
                    )}
                    <div className="parsed-list">
                      {parseResult.slides.map((slide) => {
                        const validation = slideValidation(slide);
                        return (
                          <div className="parsed-row enhanced-parsed-row" key={slide.number}>
                            <span className="parsed-number">{String(slide.number).padStart(2, '0')}</span>
                            <div className="parsed-main">
                              <strong>{slide.title || 'Sin título'}</strong>
                              <div className="field-chips">
                                <span className={validation.title ? 'ok' : 'missing'}>TÍTULO</span>
                                <span className={validation.body && validation.bodyBulleted ? 'ok' : 'missing'}>CUERPO</span>
                                <span className={validation.content && validation.contentBulleted ? 'ok' : 'missing'}>CONTENIDO</span>
                                <span className={validation.reading ? 'ok' : 'missing'}>LECTURA</span>
                              </div>
                            </div>
                          </div>
                        );
                    ...
                    <button className="primary-button" onClick={saveContentProject} disabled={!!parseResult.errors.length}>
                      {project ? 'Actualizar proyecto y volver a grabación' : 'Crear proyecto y grabar'}
                    </button>
                  </>
                )}
              </div>
          ...
          <section className="recording-screen">
            <div className="slide-header">
              ...
              <div className="slide-title">
                <span>DIAPOSITIVA {currentSlide.number} DE {project.slides.length}</span>
            ...
            <div className="slide-rail">
              {slideWindow.map((slide) => {
                ...
                const readingMissing = !slide.reading?.trim();
                return (
                  <button
                    key={slide.number}
                    className={`${index === currentSlideIndex ? 'current' : ''} ${state} ${readingMissing ? 'reading-missing' : ''}`}
                    onClick={() => goToSlide(index)}
                    disabled={busy}
                    title={readingMissing ? 'Falta LECTURA' : ''}
                  >
                    <span>{slide.number}</span>
                    <small>{readingMissing ? '!' : state === 'accepted' ? '✓' : state === 'review' ? '◐' : '○'}</small>
                  </button>
                );
              })}
            </div>
            ...
            <div className="recording-workspace">
              <div className="camera-card">
                  ...
                  <div className="device-inline">
                    {mode === 'video' && (
                      <select value={cameraId} onChange={(event) => changeCamera(event.target.value)} disabled={busy || status === 'stopped'}>
                        ...
                        {devices.cameras.map((device, index) => <option key={device.deviceId || index} value={device.deviceId}>{deviceLabel(device, 'Cámara', index)}</option>)}
                      </select>
                    )}
                    <select value={microphoneId} onChange={(event) => changeMicrophone(event.target.value)} disabled={busy || status === 'stopped'}>
                      ...
                      {devices.microphones.map((device, index) => <option key={device.deviceId || index} value={device.deviceId}>{deviceLabel(device, 'Micrófono', index)}</option>)}
                    </select>
                  </div>
                ...
                <div className="record-footer">
                  ...
                  <div className="record-actions">
                    ...
                    {status === 'ready' && (
                      <button className="record-button" onClick={startRecording} disabled={!currentHasReading} title={!currentHasReading ? 'Falta LECTURA' : ''}>
                        <span />Grabar
                      </button>
                    )}
              ...
              <aside className={`prompter-card ${!currentHasReading ? 'missing-reading' : ''}`}>
                <div className="prompter-heading">
                  <div><span className="eyebrow">PROMPTER</span><strong>LECTURA</strong></div>
                  <div className="prompter-controls"><button onClick={() => setPrompterFontSize((size) => Math.max(16, size - 2))}>A−</button><button onClick={() => setPrompterFontSize((size) => Math.min(44, size + 2))}>A+</button></div>
                </div>
                <div className="speed-row"><span>Velocidad</span><input type="range" min="10" max="90" value={prompterSpeed} onChange={(event) => setPrompterSpeed(Number(event.target.value))} disabled={!currentHasReading} /><strong>{prompterSpeed}</strong></div>
                <div className="prompter-text reading-only" ref={prompterRef} style={{ fontSize: `${prompterFontSize}px` }}>
                  {currentHasReading ? (
                    <><pre>{currentSlide.reading}</pre><div className="prompter-end">FIN DE DIAPOSITIVA</div></>
                  ) : (
                    <div className="prompter-missing-message">
                      <strong>Falta LECTURA</strong>
                      <span>CUERPO y CONTENIDO son para la diapositiva. Agrega LECTURA para indicar exactamente lo que vas a leer.</span>
                      <button className="secondary-button" onClick={() => navigate('content')}>Ir a Contenido</button>
                    </div>
                  )}
                </div>
                <div className="prompter-footer">
                  <button className="secondary-button" onClick={resetPrompter} disabled={!currentHasReading}>↥ Inicio</button>
                  <button className="primary-button small" onClick={() => setPrompterRunning((running) => !running)} disabled={!currentHasReading}>{prompterRunning ? 'Pausar' : '▶ Iniciar'}</button>
                </div>
              </aside>
            </div>
          ...
          <section className="project-screen">
            <div className="screen-heading">
              <div>
                ...
                <h1>{project.name}</h1>
                <p>{project.slides.length} diapositivas · {acceptedCount} aceptadas · {reviewCount} por revisar · {missingReadingCount} sin LECTURA</p>
              </div>
              <button className="primary-button small" onClick={() => navigate('recording')}>Continuar grabando</button>
            </div>
            <div className="project-list">
              {project.slides.map((slide, index) => {
                ...
                const readingMissing = !slide.reading?.trim();
                return (
                  <button className={`project-row ${state} ${readingMissing ? 'reading-missing' : ''}`} key={slide.number} onClick={() => goToSlide(index)}>
                    <span className="slide-number">{String(slide.number).padStart(2, '0')}</span>
                    <div>
                      <strong>{slide.title}</strong>
                      <small>{readingMissing ? 'Falta LECTURA' : state === 'accepted' ? 'Aceptada' : state === 'review' ? 'Grabada · falta aceptar' : 'Pendiente'}</small>
                    </div>
                    <span className="row-state">{readingMissing ? '!' : state === 'accepted' ? '✓' : state === 'review' ? '◐' : '○'}</span>
                  </button>
                );
              })}
            </div>
        ...
        {view === 'result' && project && (
          <section className="result-screen">
            ...
            <div className="result-summary">
              <div><strong>{acceptedCount}</strong><span>Aceptadas</span></div>
              <div><strong>{project.slides.length - acceptedCount}</strong><span>Pendientes</span></div>
              <div><strong>{missingReadingCount}</strong><span>Sin LECTURA</span></div>
              <div><strong>{project.slides.length}</strong><span>Total</span></div>
            </div>
            <div className="result-progress"><div style={{ width: `${project.slides.length ? (acceptedCount / project.slides.length) * 100 : 0}%` }} /></div>
            {acceptedCount === project.slides.length ? (
              <div className="result-ready"><strong>Todas las diapositivas están listas.</strong><span>La siguiente fase será definir exactamente cómo quieres unir y producir el video final.</span></div>
            ) : (
              <div className="result-ready"><strong>Faltan {project.slides.length - acceptedCount} diapositivas.</strong><span>Completa las tomas pendientes antes de la unión final.</span><button className="primary-button small" onClick={() => navigate('project')}>Ver pendientes</button></div>
            )}
          </section>
        )}
      </main>
      ...
      {(error || notice) && (
        <div className={`studio-toast ${error ? 'error' : 'success'}`}>
